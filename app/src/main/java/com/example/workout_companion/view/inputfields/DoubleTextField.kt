 ...
 *
 * @return True if it can convert to a double, false otherwise
 */
fun <T> isValidDouble(input: T) : Boolean {
    val stringData = input.toString()

    // Empty strings will count as valid input
    return stringData.isEmpty() or (stringData.toDoubleOrNull() != null)
}

/**
 * An OutlinedTextField that only accepts double inputs, and uses the numeric keyboard option.
 *
