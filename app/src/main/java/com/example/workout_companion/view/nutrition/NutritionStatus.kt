    ...
    if (dailyTotals != null) {
        Column(modifier = Modifier.padding(bottom = 30.dp)) {
            if(planStatus.value==NutritionStatusEnum.ABOVE_TARGET) {
                Row { NutritionStatusViolation(NutritionStatusEnum.ABOVE_TARGET) }
            }
            else{
...
@Composable
fun NutritionStatusViolation(state: NutritionStatusEnum) {
    val colorMap = mapOf(
        state to Color.Red
    )
    //Text("Status: ,${planStatus.value.descName}",
    Row(
        ...
        Text("Status: ")
        Text(state.descName, color = colorMap.getOrDefault(state, Color.Black))
    }
}